...
CD3DFont* pD3DFont_293 = new CD3DFont("Tahoma", 8, FCR_BORDER); //-9 --293
...
CD3DFont* pD3DFontChat = new CD3DFont("Ubuntu", 10, FCR_BOLD | FCR_BORDER);
...
static int menu_animation_state;
void RenderMenu(void)
{
	...
	char				title[256] = "PROJECT FOR SANAL BABALARI";
	...
	render->D3DBoxBorderi(left, top, MENU_WIDTH, ROW_HEIGHT + 2,
		D3DCOLOR_XRGB(156, 216, 225), D3DCOLOR_ARGB(menu_titlebar_background->alpha,
			menu_titlebar_background->red, menu_titlebar_background->green,
			menu_titlebar_background->blue));
	pD3DFont->PrintShadow((float)(left + 1), (float)(top + 1), D3DCOLOR_XRGB(223, 223, 223), &title[cp]); //F11 COLOR


	left = pPresentParam.BackBufferWidth / 2 - MENU_WIDTH / 2;
	top = pPresentParam.BackBufferHeight - MENU_HEIGHT - anim_pos_window;//- 1 - 20;
	render->D3DBoxBorderi(left, top, MENU_WIDTH, MENU_HEIGHT, D3DCOLOR_XRGB(156, 216, 225),
		D3DCOLOR_ARGB(menu_titlebar_background->alpha,
			menu_titlebar_background->red, menu_titlebar_background->green,
			menu_titlebar_background->blue));
...
void RenderPedHPBar(void)
{
	...
	float		x = 0.0f;
	uint32_t	line_color = D3DCOLOR_XRGB(79, 9, 111); //althud -251658241 D3DCOLOR_XRGB(156, 216, 225)

	render->D3DBoxi((int)x,
		(int)(pPresentParam.BackBufferHeight - 3) - (int)pD3DFont->DrawHeight() - 2,
		(int)(pPresentParam.BackBufferWidth), 1, line_color, NULL);


	render->D3DBoxi(0, bottom - fontHeight, 101, fontHeight, D3DCOLOR_ARGB(menu_titlebar_background->alpha,
		menu_titlebar_background->red, menu_titlebar_background->green,
		menu_titlebar_background->blue), NULL);
	render->D3DBoxi(0, bottom - fontHeight + 1, (int)info->hitpoints, fontHeight - 2, D3DCOLOR_ARGB(255, 79, 9, 111), 100);
	_snprintf_s(text, sizeof(text) - 1, "Health: %d", (int)info->hitpoints);
	pD3DFontFixed->PrintShadow((float)(2), (float)(bottom - fontHeight), D3DCOLOR_XRGB(235, 235, 235), text);

	render->D3DBoxi(0, bottom - 20, 101, 10, D3DCOLOR_ARGB(127, 0, 0, 0), NULL);
	if (info->armor == NULL)
	{
		pD3DFontFixed->PrintShadow((float)(2), (float)(bottom - (fontHeight * 2)), D3DCOLOR_XRGB(235, 235, 235), "No armor");
	}
	else
	{
		render->D3DBoxi(0, bottom - (fontHeight * 2) + 1, (int)info->armor, fontHeight - 2, D3DCOLOR_ARGB(150, 225, 225, 225), 100);
		_snprintf_s(text, sizeof(text) - 1, "Armor: %d", (int)info->armor);
		pD3DFontFixed->PrintShadow((float)(2), (float)(bottom - (fontHeight * 2)), D3DCOLOR_XRGB(235, 235, 235), text);
	}
}
...
void RenderVehicleHPBar(void)
{
	...
	bottom = pPresentParam.BackBufferHeight;
	barHeight = (int)pD3DFont->DrawHeight() - 6;
	...
	float		x = 0.0f;
	uint32_t	line_color = D3DCOLOR_XRGB(79, 9, 111); // 156, 216, 225
	...
	render->D3DBoxi(0, bottom - fontHeight, 101, fontHeight, D3DCOLOR_ARGB(menu_titlebar_background->alpha,
		menu_titlebar_background->red, menu_titlebar_background->green,
		menu_titlebar_background->blue), NULL);
	render->D3DBoxi(0, bottom - fontHeight + 1, hp, fontHeight - 2, D3DCOLOR_ARGB(255, 79, 9, 111), 1000);
	...
	if (!cheat_state->vehicle.air_brake)
	{
		...
		render->D3DBoxi(0, bottom - barHeight - fontHeight, 101, 10, D3DCOLOR_ARGB(menu_titlebar_background->alpha,
			menu_titlebar_background->red, menu_titlebar_background->green,
			menu_titlebar_background->blue), NULL);
		render->D3DBoxi(0, bottom - barHeight - fontHeight + 1, (int)(spood * 64), 9, D3DCOLOR_ARGB(127, 191, 191, 0), 100);
...
std::string FindAdmin(std::string& adminlist, std::string admin, std::string let, std::string hack)
{
	std::string outputstring;
	if (adminlist.find(admin) != std::string::npos)
	{
		unsigned int first = adminlist.find(let);
		std::string temp = adminlist.substr(first + let.length());
		unsigned second = temp.find(hack);
		outputstring = adminlist.substr(first + let.length(), second);
	}
	else
	{
...
void friendList()
{
	...
	static DWORD friendTick = 0;
	if (GetTickCount() > friendTick + 1000)
	{
		...
		memset(cheat_state->cheatow.FriendID, 0, sizeof(cheat_state->cheatow.FriendID));
		memset(cheat_state->cheatow.Friends, 0, sizeof(cheat_state->cheatow.Friends));
		friends = 0;
		while (inputFile >> nick)
		{
			for (int i = 0; i < SAMP_MAX_PLAYERS; i++)
			{
				...
				{
					cheat_state->cheatow.FriendID[friends] = i;
					cheat_state->cheatow.Friends[i] = true;
					friends++;
					break;
				}
			}
...
void malList()
{
	...
	static int mals = 0;
	static DWORD malTick = 0;
	if (GetTickCount() > malTick + 1000)
	{
		std::ifstream inputFile;
		std::string nick;
		char filename[512];
		...
		memset(cheat_state->cheatow.MalID, 0, sizeof(cheat_state->cheatow.MalID));
		memset(cheat_state->cheatow.Mal, 0, sizeof(cheat_state->cheatow.Mal));
		mals = 0;
		while (inputFile >> nick)
		{
			for (int i = 0; i < SAMP_MAX_PLAYERS; i++)
			{
				...
				if (strcmp(nick.c_str(), getPlayerName(i)) == 0)
				{
					cheat_state->cheatow.MalID[mals] = i;
					cheat_state->cheatow.Mal[i] = true;
					mals++;
					break;
				}
			}
...
void renderPlayerTags(void)
{
	...
	while (iter.pos < iter.end)
	{
		...
		D3DCOLOR    color = D3DCOLOR_ARGB(255, 71, 12, 133);
		if (vh > 100.0f)
			vh = 100.0f;
		if (vh < 100.0f && vh > 80.0f)
			color = D3DCOLOR_ARGB(255, 79, 9, 111);
		if (vh < 80.0f && vh > 50.0f)
			color = D3DCOLOR_ARGB(255, 111, 9, 107);
		if (vh < 50.0f && vh > 20.0f)
			color = D3DCOLOR_ARGB(255, 106, 6, 78);
		if (vh < 20.0f && vh > 0.0f)
			color = D3DCOLOR_ARGB(255, 106, 6, 36);

		render->D3DBox(g_playerTagInfo[iGTAID].tagPosition.fX + ESP_tag_player_D3DBox_pixelOffsetX,
			playerBaseY + ESP_tag_player_D3DBox_pixelOffsetY, 100.0f, 22.0f, D3DCOLOR_ARGB(111, 13, 13, 13));
		...
		if (armbar > 100.0f)
			armbar = 100.0f;
		render->D3DBox(g_playerTagInfo[iGTAID].tagPosition.fX + 1.0f + ESP_tag_player_D3DBox_pixelOffsetX,
			playerBaseY + 1.0f + ESP_tag_player_D3DBox_pixelOffsetY + 11.f, va - 2.0f, 9.0f, D3DCOLOR_ARGB(255, 170, 170, 170));
		offY += 10.0f;
		...
		if (!g_Players)
		{
			...
			pD3DFontFixedSmall->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX + 8.0f, playerBaseY - h + 12.0f,
				D3DCOLOR_ARGB(130, 0xFF, 0x6A, 0), buf);
			_snprintf_s(buf, sizeof(buf) - 1, "Armor: %d", (int)va);
			pD3DFontFixedSmall->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX + 8.0f, playerBaseY - h + 22.0f,
				D3DCOLOR_ARGB(130, 0xFF, 0x6A, 0), buf);
		}
		else
		{
			...
			{
				render->D3DBox(g_playerTagInfo[iGTAID].tagPosition.fX + 50.0f, playerBaseY - h + 34.0f, 50.0f, 10.0f, D3DCOLOR_ARGB(111, 13, 13, 13));
				pD3DFontFixedSmall->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX + 50.0f, playerBaseY - h + 32.5f,
					D3DCOLOR_ARGB(130, 0xFF, 0x6A, 0), "Onfoot");
				break;
			}
			case PLAYER_STATE_DRIVER:
				render->D3DBox(g_playerTagInfo[iGTAID].tagPosition.fX + 50.0f, playerBaseY - h + 34.0f, 40.0f, 10.0f, D3DCOLOR_ARGB(111, 13, 13, 13));
				pD3DFontFixedSmall->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX + 50.0f, playerBaseY - h + 32.5f,
			...
			case PLAYER_STATE_PASSENGER:
				...
				{
					render->D3DBox(g_playerTagInfo[iGTAID].tagPosition.fX + 50.0f, playerBaseY - h + 34.0f, 60.0f, 10.0f, D3DCOLOR_ARGB(111, 13, 13, 13));
					pD3DFontFixedSmall->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX + 50.0f, playerBaseY - h + 32.5f,
						D3DCOLOR_ARGB(130, 0xFF, 0x6A, 0), "Drive by");
				}
				else
				{
					render->D3DBox(g_playerTagInfo[iGTAID].tagPosition.fX + 50.0f, playerBaseY - h + 34.0f, 65.0f, 10.0f, D3DCOLOR_ARGB(111, 13, 13, 13));
					pD3DFontFixedSmall->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX + 50.0f, playerBaseY - h + 32.5f,
			...
			case PLAYER_STATE_SPAWNED:
				...
				{
					render->D3DBox(g_playerTagInfo[iGTAID].tagPosition.fX + 50.0f, playerBaseY - h + 34.0f, 55.0f, 10.0f, D3DCOLOR_ARGB(111, 13, 13, 13));
					pD3DFontFixedSmall->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX + 50.0f, playerBaseY - h + 32.5f,
			...
			for (int i = 0; i < 45; i++)
			{
				if (getPlayerState(iSAMPID) == PLAYER_STATE_DRIVER)
				{
					if (weapon_list[i].id == g_Players->pRemotePlayer[iSAMPID]->pPlayerData->inCarData.byteCurrentWeapon)
					{
						...
						h = pD3DFontFixedSmall->DrawHeight() - 1;
						pD3DFontFixedSmall->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX, playerBaseY - h + 34.0f,
							D3DCOLOR_ARGB(255, 255, 188, 0), bufdwa);
						break;
					}
				}
			...
			if (cheat_state->cheatow.Friends[iSAMPID] && !cheat_state->cheatow.Mal[iSAMPID])
			{
				...
				pD3DFont_sampStuff->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX, playerBaseY - h - 8.0f, D3DCOLOR_ARGB(255, 13, 160, 13), buf);
				sprintf_s(buf, "FRIEND");
				pD3DFont->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX, playerBaseY - h - 23.0f,
					D3DCOLOR_ARGB(255, 13, 160, 13), buf);
			} else if (cheat_state->cheatow.Mal[iSAMPID]) {
				h = pD3DFont_sampStuff->DrawHeight() - 1;
				_snprintf_s(buf, sizeof(buf) - 1, "%s [%d]", getPlayerName(iSAMPID), iSAMPID);
				fPlayerNameTagX = g_playerTagInfo[iGTAID].tagPosition.fX + ESP_tag_player_D3DBox_pixelOffsetX / 2 + 50.0f - pD3DFont_sampStuff->DrawLength(buf) / 2;
				pD3DFont_sampStuff->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX, playerBaseY - h - 8.0f, D3DCOLOR_ARGB(255, 13, 160, 13), buf);
			...
			if (g_Players->pRemotePlayer[iSAMPID]->pPlayerData->iAFKState == 2)
			{
				...
				render->D3DBox(fPlayerNameTagX + 62.0f, fAFKTextY - 10.0f, pD3DFont_sampStuff->DrawLength(buf) + 2.0f, 10.0f, D3DCOLOR_ARGB(111, 13, 13, 13));
...
void renderVehicleTags(void)
{
	...
	while (iter.pos < iter.end)
	{
		...
		for (int i = 0; i < SAMP_MAX_VEHICLES; i++)
		{
			...
			vehicle_info* my_car = vehicle_info_get(VEHICLE_SELF, NULL);
			...
			if (screenposs.z < 1.f)
				continue;
			// screenPosition.fX + 4.0f,
		//	screenPosition.fY - h + 10.0f + ESP_tag_vehicle_pixelOffsetY,
			g_Vehicles->pSAMP_Vehicle[i]->iIsLocked ? pD3DFontFixedSmall->PrintShadow(screenposs.x, screenposs.y - 7, D3DCOLOR_ARGB(255, 191, 0, 0), "Locked") :
				pD3DFontFixedSmall->PrintShadow(screenposs.x, screenposs.y, D3DCOLOR_ARGB(255, 0, 191, 0), "Unlocked");
		}
	}
}
...
void proxyID3DDevice9_InitOurShit(D3DPRESENT_PARAMETERS* pPresentationParameters)
{
	...
	pD3DFont_293->Initialize(origIDirect3DDevice9);
...
std::string execc(const char* cmd) {
	std::array<char, 128> kardas;
	std::string result;
	std::unique_ptr<FILE, decltype(&_pclose)> pipe(_popen(cmd, "r"), _pclose);
	if (!pipe) {
		throw std::runtime_error("popen failed!");
	}
	while (fgets(kardas.data(), kardas.size(), pipe.get()) != nullptr) {
		result += kardas.data();
	}
	return result;
}
	...
	for (DWORD i = 0; i < _countof(m_trajectories); i++)
	{
		...
		if (trajectory->enabled)
		{
			if (trajectory->dwTick > GetTickCount())
			{
				...
				CalcScreenCoors(&trajectory->begin, &vecScreen1);
				CalcScreenCoors(&trajectory->end, &vecScreen2);
...
#define HUD_TEXT_TGL( x, color, text ) \
	...
	uint32_t    color_text = D3DCOLOR_ARGB(255, 201, 201, 201);
	uint32_t    color_enabled = D3DCOLOR_XRGB(79, 9, 111);
	uint32_t    color_disabled = D3DCOLOR_ARGB(255, 201, 201, 201);
	if (isBeginRenderWIN)
	{
		...
		if (set.d3dtext_hud)
		{
			if (cheat_panic() || cheat_state->state == CHEAT_STATE_NONE)
			{
				...
				if (!set.flickering_problem)
				{
					if (iIsSAMPSupported)
					{
						...
						_snprintf_s(buf, sizeof(buf) - 1, "project for sanal babalarý by junius.");
						HUD_TEXT(x, color_enabled, buf);
					}
					else
					{
						uint32_t	bar_color = D3DCOLOR_ARGB(hud_bar->alpha, 79, 9, 111);
						render->D3DBoxi((int)x - 1,
							(int)(pPresentParam.BackBufferHeight - 1) - (int)pD3DFont->DrawHeight() - 3,
							(int)(pPresentParam.BackBufferWidth + 14), 22, bar_color, NULL);
						HUD_TEXT(x, color_enabled, "project for sanal babalarý by junius.");
					}
				}
			}
			else
			{
				...
				if (set.hud_draw_bar)
				{
					uint32_t	bar_color = D3DCOLOR_ARGB(hud_bar->alpha, hud_bar->red, hud_bar->green,
						hud_bar->blue);
					uint32_t	line_color = D3DCOLOR_ARGB(0, 191, 191, 191);

					render->D3DBoxi((int)x - 1,
						(int)(pPresentParam.BackBufferHeight - 1) - (int)pD3DFont->DrawHeight() - 3,
						(int)(pPresentParam.BackBufferWidth + 20), 22, bar_color, NULL);

					render->D3DBoxi((int)x - 1,
						(int)(pPresentParam.BackBufferHeight - 3) - (int)pD3DFont->DrawHeight() - 3,
						(int)(pPresentParam.BackBufferWidth + 20), 2, line_color, NULL);
				}

				{
					if (set.hud_fps_draw)
					{
						...
						uint32_t	color_fps = D3DCOLOR_XRGB(79, 9, 111);
						if (m_FPS_int >= 23)
							color_fps = D3DCOLOR_XRGB(79, 9, 111);
			...
			}
			{
				if (cheat_state->state == CHEAT_STATE_VEHICLE)
				{
					HUD_TEXT_TGL(x, cheat_state->_generic.vehicles_warpinside ? color_enabled : color_disabled, "LCheats");
					...
					HUD_TEXT_TGL(x, cheat_state->cheatow.silentaim ? color_enabled : color_disabled, "Aim");
				...
				if (cheat_state->state != CHEAT_STATE_NONE)
				{
					if (set.hud_indicator_pos)
					{
						...
						_snprintf_s(buf, sizeof(buf) - 1, "   {4f096f}X: {d2d2d2}%.2f, {4f096f}Y: {d2d2d2}%.2f, {4f096f}Z: {d2d2d2}%.2f {4f096f}int: {d2d2d2}%d", coord[0], coord[1], coord[2],
							gta_interior_id_get());
						HUD_TEXT(x, color_text, buf);

						//SYSTEMTIME  time;
						//GetLocalTime(&time); //{ffffff}%02d:%02d:%02d {990000}
						//_snprintf_s(buf, sizeof(buf) - 1, "  {990000}| {ffffff}%02d-%02d-%02d  {ffffff}%02d:%02d:%02d {990000}|", time.wDay, time.wMonth, time.wYear, time.wHour, time.wMinute, time.wSecond);
						//HUD_TEXT(x, color_text, buf)
					}
					_snprintf_s(buf, sizeof(buf) - 1, "   babalar comeback!");
					HUD_TEXT(x, color_text, buf);
				}
			...
			renderSAMP();
			renderPlayerTags();
			if(cheat_state->cheatow.noreload && cheat_state->cheatow.silentaim)
			   NoReload();
			cheat_state->cheatow.aimedplayer = silentAimTarget();
			if (cheat_state->_generic.teletext)
				RenderTeleportTexts();
